...
{  
   // Determine standard deviation of the first 100 samples and the second 100 samples.
   // We will use the range with the smaller standard deviation to determine the pedestal for the waveform.
...
void getPedestal(TH1F* hSignal, float &pedestal, float &stdv) {
   // Determine standard deviation of the first 100 samples and the second 100 samples.
   // We will use the range with the smaller standard deviation (stdv) to determine the pedestal for the waveform.
   pedestal = stdv = 0;
...
// Further: The details of the read data depends on gCONFIG.nChannelsPerBoard.
   ...
   if (gCONFIG.firstOfRun || not lastPos || not eventSize) {
	 ...
	 // WDB event encoding
         ...
         int nChannelsTot = 0;
         for (int nCh : gCONFIG.nChannelsPerBoard) nChannelsTot += nCh;
   ...
   for (unsigned int board = 0; board < gCONFIG.nChannelsPerBoard.size(); ++board) {
	 ...
	 if (tCell) {
	    for (unsigned int i = 0; i < tCell->size(); ++i) tCell->at(i) = *(unsigned short*)(word + 2);
	 }
	 ...
	 if (gCONFIG.runMode == 1) {
	    ...
	    DEBUG << "Trigger cell: " << *(unsigned short*)(word + 2) << std::endl;
	    if (tCell) tCell->at(index) = *(unsigned short*)(word + 2);
	 }
...
//		- integrationWindows (in case runMode >= 1)
   ...
   file->seekg(4, file->cur); //Skip the 'TIME' time header
   file->read(word, 4);
...
// Writes the Sample Signal Histos to the open root file
   ...
   int nChannelsTot = 0;
   char name[32];
   for (int i : gCONFIG.nChannelsPerBoard) nChannelsTot += i;
   ...
   for (int i = 0; i < nChannelsTot; ++i) wfData.at(i) = new float[SAMPLES_PER_WAVEFORM];
   ...
   if (retVal < 2) {
	 ...
	 Debug << "peak : " << peak << " bin: " << peakBin << std::endl;
         if (peak < pedestal - 5 * pedStdv) {
            ...
            if (iw.start < 0) iw.start = 0;
            if (iw.stop > SAMPLES_PER_WAVEFORM - 1) iw.stop = SAMPLES_PER_WAVEFORM - 1;
         } else {
   ...
   for ( float* f : wfData ) delete[] f;
   for (int i = 0; i < nChannelsTot; ++i) hSampleSig[i]->Write("", TObject::kOverwrite);
   delete [] hSampleSig; // Deleting the pointers to the histos, not the histos themselves
...
// Return: returns 0 on success, 1 otherwise
   ...
   if (gCONFIG.debug) theTree->Print();
   ...
   while (not file->eof()) {
	 ...
	 if (gCONFIG.sigWF == 1) {
	    for (int i = 0; i < SAMPLES_PER_WAVEFORM; ++i) aWF[i] *= -1;
	 ...
	 for (int i = iw.start; i < iw.stop && not isSignal; ++i) {
	    if ( (ped - aWF[i]) > 5 * stdv ) isSignal = true;
	 }
...
int Config() {
   int ret = 0;
   enum position {posNSAMPLE, posNSAVE, posSIGN, posCF, posLE, posRAISE, posDECAY, posNOISE, posFREQ, posDEBUG, posCONTINUE, posCANCEL};
   int pos = posCONTINUE;
   int key = 0;
   ...
   while (key != 0xA || pos < posCONTINUE) {
      ...
      standend();
      if (pos == posCONTINUE) standout();
      mvaddstr(8 + posCANCEL, 4, "continue");
      standend();
      if (pos == posCANCEL) standout();
      ...
      key = getch();
      // 0xA = Enter
      while (key != 0xA && key != KEY_DOWN && key != KEY_UP && key != KEY_LEFT && key != KEY_RIGHT) key = getch();
	 ...
	 while ((key = getch()) != 0xA && key != 0x1B) {
	    ...
	    if (n > -1 && n < 10) {
	       ...
	       for (unsigned int i = 0; i < nr.size() && not hasDecimal; ++i) hasDecimal = (nr.at(i) == -1);
	       if (not hasDecimal) {
	          nr.push_back(-1); 
	          addch(key);
	       }
...
// Deal with user input and call other functions accordingly
   ...
   for (unsigned int i = 0; i < fileList.size(); ++i) {
      ...
      if (pos != std::string::npos) outFileName.replace(0, pos  + 1, "");
