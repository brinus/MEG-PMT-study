...
void mScan_HV(){
    ...
    std::string fName = "scanHV.";
    std::string fType = ".root";
    std::vector<std::string> vVolt = {"10", "20", "30"};

    TH1 * h1 = new TH1F();
    TFile * f = new TFile();
    TTree * t = new TTree();

    for (std::string volt : vVolt){
        f = new TFile((fName + volt + fType).c_str()); 
        t = (TTree *)f->Get("T");
        t->Draw("area >> hArea");
    ...
    for (Int_t i = 0; i < vHist.size(); i++){
        ...
        legend->AddEntry(vHist[i], ("HV [V] =  " + vVolt[i]).c_str(), "E");
    }
